function out = makeLineChart(mytable,myChartOptions)
...
s{end+1} = sprintf(']); var options = {hAxis: {textStyle: {fontName: ''Arial''}},isStacked: %s};var chart = new google.visualization.LineChart(document.getElementById(''chart_div%d''));chart.draw(data, options);}</script></head><body><div id="chart_div%d" style="width: 700px; height: 200px;"></div></body></html>',myChartOptions.isStacked,aux_rand,aux_rand);
